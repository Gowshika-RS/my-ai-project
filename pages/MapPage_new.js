    ...
    const initMap = async () => {
      ...
      const onInspect = async (lat, lng) => {
        try {
          ...
          const trend = json.trend || deterministicTrend(score);  // Use backend trend if available
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          const level = json.risk_level || json.level || (score > 70 ? 'High' : score > 30 ? 'Medium' : 'Low');
          const peakHours = json.peak_hours || json.peakHours || deterministicTrend(score).map((v,i) => `${17+i}:00-${18+i}:00`);
          const trend = json.trend || deterministicTrend(score);  // Use backend trend
      ...
      <div className="row g-3">
        ...
        <div className="col-lg-4">
          <div className="risk-card">
            ...
            {loadingAnalysis ? (
              <p className="risk-muted">Analyzing location...</p>
            ) : selected ? (
              <>
